...
- 2026-07-12: WAV終端後もDelay残響を続けるため、同じ `EffectID` / 内部レイヤーで1～2フレーム以内に隣接する後続音声ObjectへDelayリングを引き継ぐ処理を追加した。元WAVの直後へ `Sample\無音_極小ノイズ_ループ推奨.wav` を置き、両方を同じ `グループ制御（音声）` の範囲に入れると、元WAV終了後もエコーが継続することを実機確認済み。重複音声や離れたObjectのContext分離は維持する。
