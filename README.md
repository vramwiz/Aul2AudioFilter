...
`スタイル` は詳細パラメーターを触らずに用途別の聞こえ方を選ぶための項目です。電話、無線、隣室、風呂、トンネル、ナレーションなどを選択できます。
パラメーター名は、AviUtl2 上で他エフェクトの項目と区別しやすいよう英語表記にしています。ただし、最上段の `スタイル` だけは利用者が直接選びやすいよう日本語表記にしています。

## 配置

- `Aul2AudioFilter.dpr` / `Aul2AudioFilter.dproj`: Delphi プロジェクト本体
- `Aul2AudioFilterPlugin.pas`: プラグイン入口、各エフェクトユニットの接続
- `Aul2AudioFilterPluginSoundStyle.pas`: スタイル系の GUI 項目、状態、音声処理
...
現在の選択肢は `なし`、`電話（弱）`、`電話（強）`、`無線（弱）`、`無線（強）`、`メガホン`、`隣室（薄い）`、`隣室（厚い）`、`遠声（近め）`、`遠声（遠め）`、`風呂（狭い）`、`風呂（広い）`、`トンネル`、`アナウンス`、`ナレーション`、`夢（薄い）`、`夢（深い）` です。
