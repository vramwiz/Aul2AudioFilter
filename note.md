...
- 再生中・出力中はData書込みだけでなく、Controllerの `CallEditSectionParam` を使うAlias取得、選択Object取得、レイヤー取得も行わない。表示開始時に選択レイヤーをキャッシュし、表示済みグラフは共有メモリの最新値だけを使う。Controller再取得要求は停止後まで保留する。
