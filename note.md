...
- `Source\Aul2AudioControllerView.pas`: Controller のVCLフォーム、エフェクター切り替え、定義に基づくパラメーターコントロールの再構成と配置を担当する。
...
### 2026-07-15 作業休止時点・再開場所

- GUIの外観はユーザー確認済みで、現状を採用する。上からエフェクター選択コンボ、赤色LED付き全面トグル、選択型パラメーター、ノブ領域の順。エフェクターごとの本来の識別色を背景に使い、ノブ周辺は暗い同系色を維持する。
- `Source\Aul2AudioControllerEffectDefinition.pas` を追加済み。エフェクター名、日本語LED表記、Use項目名、テーマ色、選択項目、ノブのAlias項目名・範囲・刻み・小数桁・単位をViewから分離した。
- メイン、ボリューム、指示線、文字の指定色は全20エフェクター分を `EFFECT_COLORS` に集約済み。ボリュームと指示線は指定色をそのまま使い、メインだけは明度上限を超えた色を必要量だけ暗くする。元から上限以下の濃色・黒系は追加で暗くしない。EQ、Compressor、Noise、Auto Gain、Noise Gate、Chorus、Limiterの明色面では文字を黒にする。
- DelayとEQは定義と実同期を接続済み。エフェクターコンボを切り替えるとLED表記、選択コンボ、ノブ数・範囲・単位・配色を再構成し、選択中Objectの対応項目を読み書きする。EQでは `Mode` / `Low Cut` / `High Cut` / `Mix` を表示し、4個目のノブを隠す。
- `Source\Aul2AudioControllerSync.pas` はDelay固定処理から、選択中のエフェクター定義を受け取る汎用処理へ変更済み。Aliasは1回だけ取得し、Use、Select、Volume項目を一括読込する。操作時は変更した1項目だけを書き込む。
- Compressorも定義と実同期へ接続済み。選択コンボなしで、`Threshold` / `Ratio` / `Attack` / `Release` / `Makeup` / `Mix` の6ノブを表示する。Viewのノブは名前付き4個から最大7個の共通配列へ整理済み。
- Voice Driveも定義と実同期へ接続済み。選択コンボなしで、`Drive` / `Body` / `Level` / `Mix` の4ノブを表示する。
- Distortionも定義と実同期へ接続済み。`Soft Clip` / `Hard Clip` の選択コンボと、`Drive` / `Tone` / `Level` / `Mix` の4ノブを表示する。
- Noiseも定義と実同期へ接続済み。`White` / `Crackle` の選択コンボと、`Level` / `Mix` の2ノブを表示する。
- Bit Crusher、Tremble、Wobbleも定義と実同期へ接続済み。順に3ノブ、3ノブ、4ノブを表示し、いずれも選択コンボは持たない。
- Pitchも定義と実同期へ接続済み。4モードの選択コンボと、定義上限となる7ノブを表示する。モード別の表示切り替えは行わない。
- Ring Mod、Muffle、Whisperも定義と実同期へ接続済み。いずれも選択コンボなしの3ノブ構成とする。
- Auto Gain、Noise Gate、Ghostも定義と実同期へ接続済み。いずれも選択コンボなしの4ノブ構成とする。
- Chorus、Reverb、Output、Limiterも定義と実同期へ接続済み。ChorusとReverbは選択コンボと4ノブ、Outputは1ノブ、Limiterは3ノブを表示する。これで全20エフェクターのパラメーター定義が接続済みとなり、プレースホルダー定義は削除した。
- Debug Win64は警告0・エラー0でビルド済み。`C:\ProgramData\aviutl2\Plugin\Aul2AudioFilter\Aul2AudioController.aux2` へ反映済み。
- 再開後の最初の作業はAviUtl2を再起動し、DelayとEQの実機切り替えを確認すること。両方について、読込、LED ON/OFF、選択コンボ、ノブドラッグ、ホイール、直接入力、元GUIへの反映を確認する。
- 次の実機確認では全20エフェクターの表示、読込、LED、選択項目、各値の書き込みを確認する。特にPitchの7ノブ複数段レイアウトと下端の収まり、ChorusのStereo Mode、ReverbのTypeを確認する。

- 新しい拡張プラグイン名は `Aul2AudioController` とする。既存の `Aul2AudioFilter` のパラメーターを外部から参照し、設定操作を補助する。音声処理自体は担当しない。
- 拡張プラグインの骨格、エフェクター選択コンボ、LEDスイッチ、ノブUIを実装済み。全20エフェクターで、選択に応じて実際のUse、選択項目、数値パラメーターを読み書きする。
...
- 有効な対象を読み込む前の初期状態は、エフェクター `OFF`、数値は各定義の最小値、選択項目は先頭とする。有効な対象を取得したら、読み込んだ値で GUI 全体を置き換える。個別のエラー表示は設けない。

### 現在の同期実装状況

- 最初の同期対象としてDelayを採用し、現在は全20エフェクターを同じ汎用同期へ接続している。`Use` は専用LEDスイッチ、選択項目はコンボボックス、連続数値は専用ボリュームコントローラーで操作する。
- ControllerのGUI領域へマウスが入った時に `WM_SETCURSOR` を契機として、選択中Objectのエイリアスを取得する。同じマウス進入中は一度だけ読み込み、領域外へ出た後の再進入で再取得する。
- エイリアス内で最初に見つかる `サウンドエフェクター` / `音声効果` / `Aul2AudioFilter` のセクションから、選択中エフェクターの定義項目を読み込む。GUI更新中はイベントを書き込みへ接続せず、読み書きの循環を防止する。
- `Dly: Stereo Mode` はインデックス値ではなく `Normal` / `Ping-Pong` の表示値として取得される。読み込み側は表示値と数値表現の両方を解釈でき、実機で正常取得を確認済み。
- GUIからの書き込みはLED、選択コンボ、ノブのすべてで接続済み。縦ドラッグ、横ドラッグ、ホイール、直接入力はいずれも変更対象の1項目だけを書き込む。
- 数値項目の値域は `Time(ms)` が `1..1000`、`Dry` と `Wet` が `0..2`、`Feedback` が `0..0.95`。現在の刻みは順に `1`、`0.01`、`0.01`、`0.01`。
...
- EQ以降もコード接続とビルドまで完了し、実機確認待ち。各エフェクター固有の項目名・値域・選択肢は `Source\Aul2AudioControllerEffectDefinition.pas` へ集約している。

### エフェクター選択

- エフェクター選択は全エフェクターを実際の音声処理順で格納したコンボボックス方式とする。`Use` のON/OFFと選択状態を分離し、OFFにしても選択肢から消さない。
- コンボボックスを閉じた状態では、ホバー中のホイール上で1つ前、下で1つ次へ移動し、先頭と末尾では停止する。ドロップダウンを開いた状態では標準リスト操作を使い、遠いエフェクターを直接選ぶ。
- 全20エフェクターで、選択変更を実パラメーター表示と同期先へ接続済み。
- エフェクターコンボはControllerの最上段へ置く。Handle生成後・フォーム表示前に `SetWindowTheme(Handle, '', '')` を適用し、初期表示からダーク配色にする。直下の `LampSwitchHost` には専用LEDスイッチを置く。Delayでは `Use` ではなく「遅延音を加える」と表示し、ON時は赤色LEDを発光させ、LEDと文字を含む領域全体で切り替える。その下へ選択項目名とコンボボックス、残りをノブ領域として上詰め配置する。デバッグ用状態ラベルはレイアウトから外して非表示にする。
- 選択中エフェクターの電源ボタン兼表示灯とパラメーター領域切り替えは、全20エフェクターで同じ汎用定義・同期へ接続済み。
...
- ノブ外側のベース部分には最小角と最大角を示す短い終端線を置き、回し切った位置が分かるようにする。ノブと下側の数値Editの間隔は狭く保ち、DPI拡大で間延びさせない。
- ノブはマウスダウン後のドラッグで操作する。上または右で増加、下または左で減少し、4px以上動いた時点で移動量の大きい方向へ操作軸を固定して斜め移動による二重加算を避ける。縦は粗調整、横は微調整とし、`VOLUME_VERTICAL_RANGE_PER_PIXEL` / `VOLUME_HORIZONTAL_RANGE_PER_PIXEL` で独立調整する。現行値は全値域に対して縦 `1/180` /px、横 `1/600` /px。ホイールはパラメーター固有の刻み1単位ずつ変更する。
- ノブ下側には小型の `TEdit` を置き、その右側へ単位を独立表示する。Enterまたはフォーカス移動で確定、Escで編集前の表示へ戻し、不正値は拒否する。値は対象パラメーターの範囲内へ制限し、刻みへ丸める。
...
- テーマ色は代表的なギターエフェクターの筐体色を基準にし、`EFFECT_COLORS` const配列へ20色を集約する。各エフェクトの1行に筐体基準色、ボリュームカード色、ノブ位置線色を置き、3色を独立調整できる。広いベース背景は筐体基準色を暗いニュートラル色へ混ぜて目の疲れを抑える。色だけに依存せず、現在のエフェクター名を最上段コンボへ常に表示する。
- エフェクターコンボの選択に応じてRootPanelを青・緑・黄・赤・紫などへ切り替える。LEDスイッチ面と各ボリュームカード面は暗い同系色を維持し、白系文字と黒いノブの視認性を残す。この配色はユーザー確認済み。
...
- `Preset` は `グループ制御（音声）` 専用のユーザープリセット機能。保存、削除、起動時読込、名前のインライン編集、タイムラインへのD&Dを提供する。操作エラーはページ内の状態ラベルへ表示し、ダイアログは使わない。
- Preset一覧は `Ctrl+Up` / `Ctrl+Down` で選択項目を上下へ移動し、変更順を即時保存する。
