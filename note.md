...
### Controller エフェクト表示の検討

- Controllerのエフェクター操作画面には、現在の設定によるエフェクトの掛かり具合を視覚的に把握できるグラフ状の表示を追加する方向で検討する。
...
- 最初は `Delay` で、配置、背景、軸、文字、補助線、密度調整、設定変更時の再描画を実装して共通基盤を確認する。次に `EQ` で曲線と対数周波数軸を確認する候補とする。
...
- `Delay` の初期模式図は追加済み。最大幅300px、高さ150pxでノブ最終行の下へ中央配置し、十分な高さがない場合は非表示にする。設定読込、Time/Dry/Wet/Feedback、Normal/Ping-Pong、Useの変更へ追従する。最初の座標軸版は実機で意味が読み取りにくかったため、反射模式図へ変更した。反射模式図版のRelease Win64ビルドは確認済みで、AviUtl2上の実表示は未確認。
