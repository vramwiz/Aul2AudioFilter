...
- 現段階では選択操作だけを実装済み。選択変更は状態ラベルへ表示するだけで、既存Delayの `Use` / `Stereo Mode` / ボリュームコントローラーには反映しない。
...
- 次段階で、選択中エフェクターの独立した電源ボタンと表示灯、パラメーター領域の切り替えを接続する。選択方式のパラメーターには通常のコンボボックスを使う。
...
- テーマ色の実機確認用として、エフェクターコンボの選択に応じてRootPanel、LEDスイッチ面、各ボリュームカード面を暗いテーマ色へ切り替える。白系文字と黒いノブの視認性を維持する低輝度値とし、現段階では見た目だけを切り替えてパラメーターや同期先はDelayのままにする。
