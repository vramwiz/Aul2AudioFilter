...
- 実装途中は `note.md` / `HISTORY.md` を逐次更新せず、表示と実機確認まで完了した後に、現状・課題整理を `note.md`、実装経緯と検証結果を `HISTORY.md` へまとめて書く。
...
- `Source\Aul2AudioControllerOutputGraph.pas`: ControllerのOutput表示。Monitor共有データからInput／Output L/R Peakと短いRMS履歴をスナップショット描画する。
...
- Controllerのエフェクター操作画面には、現在の設定によるエフェクトの掛かり具合を視覚的に把握する補助表示を置く。現在は `Delay`、`EQ`、`Compressor`、`Distortion`、`BitCrusher`、`NoiseGate`、`Muffle`、`Output`、`Limiter` を実装済み。
...
第二弾は実音声を使う表示を次の順で検討する。

| 優先 | エフェクター | 実装する主表示 | 優先理由 |
| ---: | --- | --- | --- |
| 1 | Pitch | 高解像度の前後スペクトル | ピッチ移動を横方向で確認できる。Pitch専用の解析解像度が必要。 |
| 2 | RingMod | 前後スペクトルと側波帯表示 | Muffle／Pitchで作ったスペクトル部品を再利用できる。 |
| 3 | Whisper/Breath | 前後または差分スペクトル | 追加された息成分の帯域をスペクトル部品で表示できる。 |
| 4 | Noise | Noise直前・直後の差分波形 | 差分波形取得と自動スケール表示の共通部品になる。 |
| 5 | VoiceDrive | 非線形伝達カーブ＋XY点列 | Distortionの描画方式を参考にでき、差分や倍音表示へ発展できる。 |
| 6 | Tremble | Input／Output RMS時間履歴 | 時間履歴グラフの最初の実装として比較的単純。 |
| 7 | AutoGain | Target、Input／Output RMS、補正ゲイン履歴 | Trembleで作った時間履歴部品を再利用でき、実音声表示の価値が高い。 |
| 8 | Wobble | 遅延時間の変動履歴 | 時間履歴部品を再利用できるが、前後音声からの遅延推定が必要。 |
| 9 | Reverb | Wet残響量と減衰履歴 | 入力停止後の残響エネルギーを時間履歴として表示する。 |
| 10 | Ghost | 残響の膨らみと減衰履歴 | Reverbの残響履歴部品を再利用できる。 |
| 11 | Chorus | L/R遅延変動、ステレオ相関または簡易Vectorscope | L/R別データと位相・相関表示が必要で、最も専用実装が多い。 |
...
| 完了済み | 背景・動的表示 | 推奨度 | 理由 |
| --- | --- | :---: | --- |
| Delay | 出力またはWet成分の音量エンベロープ | △ | 時間軸は一致するが、生波形では密集しやすい。薄いエンベロープなら反射位置と比較できる。 |
| EQ | Input／Outputスペクトル | ◎ | 既存グラフもスペクトルも横軸が周波数なので、設定カーブと実際に削られた帯域を直接比較できる。 |
| Compressor | Wave／Spectrumは追加しない | × | 既存グラフは入力dB×出力dBなので軸が一致しない。現在の入出力点とGain Reductionを重ねる方が有用。 |
| Distortion | Input×OutputのXY点群 | ○ | 実サンプルのXY点を薄く背後へ描くと、設定カーブと実際の歪みを同じ座標系で比較できる。 |
| BitCrusher | Input×OutputのXY点群 | ○ | 実サンプルが量子化階段のどこへ配置されたかを確認でき、生波形より既存軸と相性がよい。 |
| NoiseGate | Wave／Spectrumは追加しない | × | 現在の入力／出力レベル点、Open／Closed、適用ゲインを重ねる方が分かりやすい。 |
| Limiter | Wave／Spectrumは追加しない | × | 現在のピーク点とGain Reductionを重ねる方が、制限動作を直接確認できる。 |

- `EQ` は追加価値が最も高い。背景へ時間平滑化したInput／Outputスペクトルを同じ音量基準で表示する。Inputは暗いグレーの1px線、Outputはエフェクト色を暗くした1px線とし、現在の太い設定特性カーブを最前面へ置く。
- `Distortion` と `BitCrusher` は生波形ではなく、Xを処理前サンプル値、Yを処理後サンプル値とする暗い半透明相当のXY点群を背景へ表示し、設定カーブを最前面へ置く。
- `Delay` に追加する場合は、生波形ではなく時間区間ごとのピークまたはRMSエンベロープとする。反射模式図が読みにくくなる可能性があるため優先度は低い。
- `Compressor`、`NoiseGate`、`Limiter` はWave／Spectrumを追加せず、現在点や状態、Gain Reductionを既存グラフへ重ねる。
