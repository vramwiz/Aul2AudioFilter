...
- `Source\Aul2AudioControllerNoiseGateGraph.pas`: Controller最下部に置くNoiseGate入出力特性表示。Threshold未満のFloor減衰、抑制領域、ゲートが開く段差、Attack／Release情報の描画を担当する。
- `Source\Aul2AudioControllerLimiterGraph.pas`: Controller最下部に置くLimiter入出力特性表示。Ceilingまでの無加工線、超過部分のピーク制限、Mix、抑制領域、Release情報の描画を担当する。
...
- `Source\Aul2AudioViewRenderSpectrumWaterfall3D.pas`: `Spectrum Waterfall (3D)` の独立スペクトラム帯、流動履歴、編集時形状保持、3D直接描画を担当する。
...
- `Source\Aul2AudioViewRenderVectorscopeTrail3D.pas`: `Vectorscope Trail (3D)` のレイヤー別履歴抽出、履歴平滑化、3D点列リボン・点タイル描画を担当する。
...
- Controllerのエフェクター操作画面には、現在の設定によるエフェクトの掛かり具合を視覚的に把握する補助表示を置く。現在は `Delay`、`EQ`、`Compressor`、`Distortion`、`BitCrusher`、`NoiseGate`、`Limiter` を実装済み。
...
- Controller内の表示は現在の設定値から作る静的なパラメータービジュアライザーとし、共有メモリ、音声サンプル、再生位置、履歴、監視タイマーには依存させない。ノブ、選択項目、エフェクターの変更時と、選択Objectから設定を読み直した時だけ更新する。
- マルチエフェクター全体の実際の入出力や時系列変化が必要になった場合は、Controllerへ追加せず、Monitorへ共有メモリ経由で値を送り、Monitorのタイマーで監視して描画する。
- エフェクターごとに描画内容は基本的に分ける。同じ表現を使える場合は、グラフ枠、軸、目盛り、補助線、周波数特性、入出力伝達、時間変化などの基本描画部品を再利用する。
- `Delay` で配置、背景、文字、補助線、密度調整、設定変更時の再描画を確認し、`EQ` で曲線、塗り、対数周波数軸、カット位置ラベルを確認済み。ほかのエフェクターへの追加は今後の別作業とする。
...
- `Delay` の反射線はDPIに応じた約4pxの太線として明確に表示し、補助線、基準線、頂点を結ぶ減衰線は1pxにする。DryとEchoは色を分け、文字は `Dry`、`Echo`など短い直接表記にする。描画本数は実描画幅と最小レベルから1～6本程度に制限し、線や文字を密集させない。`Ping-Pong` はL/Rを色と短い文字で区別する。
- `Delay` の文字は通常UIに近い9ptと明るい色を基本にし、狭い表示だけ8ptへ戻す。OFF時も文字の明るさは維持して設定内容を読めるようにする。右下には最後に表示しているEchoの時刻を表示し、反響が続く時間の目安にする。
- `Delay` の反射模式図は追加済み。最大幅300px、高さ150pxでノブ最終行の下へ中央配置し、十分な高さがない場合は非表示にする。設定読込、Time/Dry/Wet/Feedback、Normal/Ping-Pong、Useの変更へ追従する。最初の座標軸版は実機で意味が読み取りにくかったため反射模式図へ変更した。Release Win64ビルドとAviUtl2上の実表示を確認済みで、厳密な処理結果ではなく調整具合の目安として採用可能な状態。
- `EQ` は線グラフを採用し、20Hz～20kHzの対数周波数軸、通過量0～100%、1pxの補助線、約3pxの特性カーブ、通過領域の薄い塗りで構成する。棒グラフは実音声スペクトラムやバンド別ゲインの表現に向くためControllerのEQ設定表示には使わない。
- `EQ` のカーブはDSP側と同じButterworth型2次High/Low Passの傾向を設定値だけから近似する。`Low Cut`、`High Cut`、`Band Pass`を切り替え、カット位置に周波数値と縦線を表示する。`Mix`は0%でフラット、100%でフィルター特性となるように補間する。Band PassでHigh CutがLow Cut以下の場合はDSP側と同様に狭い通過帯域として扱う。
...
- 2026-07-17、次に `NoiseGate` の入出力特性カーブを追加した。横軸は入力-80～0dB、縦軸はFloor減衰を収める出力-160～0dBとし、Threshold未満ではDSPと同じく入力へFloor dBを加えた線、Threshold以上では無加工線を表示する。両者の段差、抑制領域の塗り、Threshold位置、Attack、Releaseを表示する。AttackとReleaseは定常入出力カーブを変えず、ゲートが開閉するときの時間応答値として数値だけを表示する。Release Win64ビルドとAviUtl2上の実表示を確認済みとし、NoiseGateグラフを完成扱いとする。
- 2026-07-17、次に `Limiter` のピーク制限カーブを追加した。横軸と縦軸を-60～+12dBとし、Ceilingまでは無加工線、超過部分はDSPと同じくWetをCeilingへ制限し、DryとのMixを線形振幅で合成した定常出力を表示する。無加工線との差を抑制領域として塗り、Ceiling位置、Mix、Releaseを表示する。Releaseはピーク後のゲイン回復時間なので定常カーブを変えない。初回実機表示で抑制領域が背景へ沈んで見えたため、塗りをアクセント色の20%から38%へ明るくした。Release Win64ビルドと`Aul2AudioController.aux2`への反映は確認済み、塗り調整後の実表示確認は未実施。
...
| エフェクター | 表示案 | 概要 |
| --- | --- | --- |
| Delay | ディレイ反射図 | 横軸を時間として原音と遅延音を縦線で表示する。Feedbackによる繰り返しは、時間経過とともに高さが減る複数の線で表す。 |
| EQ | 周波数特性カーブ | 横軸を周波数、縦軸を通過量として、Low Cut・High Cut・Band Passで残る帯域を折れ線または曲線で表示する。 |
| Compressor | 入出力特性カーブ | 横軸を入力音量、縦軸を出力音量として、Threshold以降で傾きが変わる圧縮カーブを表示する。Ratioの強さを形へ直接反映する。 |
| VoiceDrive | 入出力振幅カーブ | 横軸を入力振幅、縦軸を出力振幅として、Driveによって波形の山が押しつぶされる形を表示する。Bodyは中央付近の太さとして反映する。 |
| Distortion | 歪み伝達カーブ | 横軸を入力振幅、縦軸を出力振幅として、Soft ClipとHard Clipのつぶれ方を表示する。Driveを上げるほど上下端を平らにする。 |
| Noise | ノイズ分布表示 | 横軸を時間として、ノイズ量を細かなランダム線で表示する。Whiteは連続、Crackleは突発的な高い点や線として表す。 |
| BitCrusher | 量子化階段カーブ | 横軸を入力振幅、縦軸を出力振幅として、滑らかな斜線が階段状になる様子を表示する。BitDepthが小さいほど段数を減らす。 |
| Tremble | 音量変調カーブ | 横軸を時間、縦軸を音量として、音量が周期的に上下する波を表示する。Rateで波の密度、Depthで上下幅を変える。 |
| Wobble | 遅延時間変動カーブ | 横軸を時間、縦軸を遅延時間として、基準Delayを中心に周期的に揺れる線を表示する。DepthとRateを形に反映する。 |
| Pitch | ピッチ変化カーブ | 横軸を時間、縦軸を音程差として表示する。Naturalなどは一定線、Stepでは設定した音程差を段階的に上下する線で表す。 |
| RingMod | 振幅変調カーブ | 横軸を時間、縦軸を変調量として、FrequencyとDepthに応じた周期波を表示する。周波数が高いほど波を密にする。 |
| Muffle | ローパス特性カーブ | 横軸を周波数、縦軸を通過量として、Cutoffより高い帯域が減衰するカーブを表示する。Amountで傾きや減衰量を変える。 |
| Whisper/Breath | 息成分スペクトル形状 | 横軸を周波数、縦軸を追加量として、息成分が加わる帯域を山型の曲線で表示する。Toneで山の位置を左右へ移動する。 |
| AutoGain | 補正ゲイン履歴 | 横軸を時間、縦軸を増減ゲインとして、自動的に加えられている補正量を表示する。中央を0dBとして、増幅と減衰を上下に示す。 |
| NoiseGate | ゲート入出力カーブ | 横軸を入力音量、縦軸を出力音量として、Threshold以下がFloorまで抑えられる形を表示する。境界位置で効き始める音量を示す。 |
| ReverseReverb/Ghost | 残響影エンベロープ | 横軸を時間、縦軸を残響量として、遅れて膨らみ、減衰する残響影を表示する。Sizeで長さ、Feedbackで後続の影を増やす。 |
| Chorus | 左右遅延変動カーブ | 横軸を時間、縦軸を遅延時間として、LとRの揺れを2本の線で表示する。Wideでは左右が反対方向へ動く形にする。 |
| Reverb | 残響減衰カーブ | 横軸を時間、縦軸を残響量として、音が徐々に減衰する曲線を表示する。RoomSizeで長さ、Dampingで高域側の減衰の速さを示す。 |
| Output | 入出力レベル比較 | 横軸を時間として、入力レベルとGain適用後の出力レベルを2本の線で重ねる。単純な上下移動として効果を示す。 |
| Limiter | ピーク制限カーブ | 横軸を入力音量、縦軸を出力音量として、Ceilingまでは直線、上限を超える部分は水平に近づく形で表示する。 |

描画方式は20種類すべてを個別実装せず、次の基本部品を使い回す。

| 基本表示 | 対象 |
| --- | --- |
| 周波数特性カーブ | EQ、Muffle、Whisper/Breath |
| 入出力伝達カーブ | Compressor、VoiceDrive、Distortion、BitCrusher、NoiseGate、Limiter |
| 時間変化カーブ | Tremble、Wobble、Pitch、RingMod、AutoGain、Output |
| 時間位置・減衰表示 | Delay、ReverseReverb/Ghost、Reverb |
| 2線比較表示 | Chorus |
| ランダム分布表示 | Noise |

- 特に小型表示と相性がよい優先候補は、`EQ`、`Compressor`、`Distortion`、`BitCrusher`、`NoiseGate`、`Limiter`、`Delay`。これらはすべて実装済み。設定値を変更すると形が明確に変わるため、実際の音声データを取得しなくても有用な表示になる。
...
- 表示タイプは `Equalizer Bars` / `Mirror Bars` / `Filled Spectrum` / `Circular Spectrum` / `Wave Line` / `Pixel Wave` / `Pulse Wave` / `Vectorscope` / `Circular Bars (3D)` / `Radial Waveform (3D)` / `Spectrum Landscape (3D)` / `Waveform Tunnel (3D)` / `Spectrum Waterfall (3D)` / `Vectorscope Trail (3D)` の 14 種類。
...
- `Spectrum Waterfall (3D)` は既存スペクトラム履歴を接続せず、Z方向へ独立した細い帯として最大32列並べる。`Solid`は周波数方向へ連続したリボン、`Blocks`は周波数方向も分割したタイル列として描く。再生中は無音を高さ0の列として流し、編集中はカーソル位置の共有履歴を再取得する。`Density`、`Spacing`、`Thickness`、`Smooth`、X/Y/Z Scale、色、周波数範囲、高域強調を反映する。`Smooth`は新しい先頭列だけでなく履歴方向の隣接帯にも適用する。円形座標を持たないため`Base Radius`は使用しない。共有メモリは追加せず、描画失敗時は`Filled Spectrum`へ戻す。Release Win64ビルド、表示、各パラメーターを実機確認済みで、完成扱いとする。
- `Vectorscope Trail (3D)` は全レイヤー共通256件のVectorscope履歴から対象レイヤーを抽出し、現在フレーム以前を新しい順に最大32層並べる。各層は処理後Output L/R代表点64組を`X=(L-R)/2`、`Y=(L+R)/2`へ変換する。`Solid`は各時刻の点列を厚み付きリボンで接続し、`Blocks`は独立した点タイルとして描く。履歴層同士は接続しない。再生中はView内の流動履歴へ有効層を追加し、無音時は中心点ではなく空層を追加して既存層を奥へ押し出す。最後の有効層が履歴分流れ切ってから透明になる。`Density`、`Spacing`、`Thickness`、`Smooth`、X/Y/Z Scale、色、`Source Layer`を反映する。通常音量でも指定解像度を使いやすくするためX/Y共通のType専用2.5倍感度を持ち、X方向は2D Vectorscopeと同じ相対10倍のSide感度も維持する。描画範囲は正方形へ制限せず、Xを指定Width、Yを指定Height基準にする。履歴層の奥行きを見やすくするためZ ScaleはType専用8倍感度を持つ。円形・スペクトラム専用設定は使用しない。共有メモリは追加せず、実際の描画失敗時は2D `Vectorscope`へ戻す。意図的な空表示ではフォールバックしない。Release Win64ビルド、表示、流れ切り、長方形座標、専用X/Y/Z感度、各パラメーターを実機確認済みで、完成扱いとする。
